...
static uint8_t _lcdcols;
static uint8_t _lcdrows;
static uint8_t _backlightval;

static int i2cdev_fd;

void i2c1602_init(uint8_t cols, uint8_t rows){
...
void i2c1602_begin(uint8_t cols, uint8_t rows, uint8_t dotsize) {
    if (rows > 1) {
...
void i2c1602_setCursor(uint8_t col, uint8_t row){
    ...
    if ( row > _lcdrows ) {
        row = _lcdrows - 1;    // we count rows starting w/0
    }
...
void i2c1602_printstr(const char *str) {
    ...
    for(int r = 0; r < _lcdrows; r++) {
        i2c1602_setCursor(0, r);
        for(int c = 0; c < 20; c++) {
            int pos = r * _lcdcols + c;
            if(0 == str[pos]) {return;}
            i2c1602_write(str[pos]);
        }
    }
}
